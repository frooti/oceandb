...
## CONFIG ##
file_path = '/var/www/dataraft.in/currentdirection_*.mat'
output_path = 'currentdirection_timeseries.data'
...
def timeseries():
	...
	for f in sorted(glob.glob(file_path), key=lambda x: int(x.split('/')[-1].split('_')[-1].split('.')[0])):
